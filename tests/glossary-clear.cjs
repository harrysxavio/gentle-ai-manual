...
(async () => {
  ...
  console.log('\nGlossary clear test — desktop (1440\u00d7900, dark)');
  {
    ...
    await check('initial counter shows total (84)', async () => {
      ...
      assert.match(text, /84 de 84 t\u00e9rminos/);
    });
    ...
    await check('search narrows results', async () => {
      const text = await page.locator('#glossary-status').textContent();
      assert.match(text, /^\d+ de 84 t\u00e9rminos$/);
      searchCount = parseInt(text.match(/^(\d+)/)[1], 10);
      assert.ok(searchCount < 84, 'search should narrow below 84, got ' + searchCount);
    });
    ...
    await check('Engram category narrows independently', async () => {
      ...
      assert.ok(c >= 10 && c < 84, 'Engram category should narrow (got ' + c + ')');
    });
