...
- **No diferenciar entre timeouts de conexión y timeouts de respuesta.** Un timeout de conexión (curl: `Connection timed out`) significa que el servidor no aceptó la conexión TCP. Un timeout de respuesta (curl: `transfer closed with bytes remaining`) significa que la conexión se estableció pero el servidor no terminó de enviar la respuesta. La causa y la solución son diferentes: el primero suele ser red/firewall, el segundo suele ser servidor sobrecargado.
